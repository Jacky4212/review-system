...
<style>
  ...
  * { margin: 0; padding: 0; box-sizing: border-box; }
  ...
  .topbar {
    ...
    position: sticky; top: 0; z-index: 99; box-shadow: 0 2px 8px rgba(0,0,0,0.15);
  }
...
'''
  ...
  <div class="section">
    ...
    <p><strong>最大项：</strong>n个变量的最大项是n个因子的<strong>或项</strong>，每个变量都以它的原变量或非变量的形式在或项中出现，且仅出现一次。n个变量的最大项应有2<sup>n</sup>个。</p>

    <h3>最小项与最大项的性质对比</h3>
    <table>
      <tr><th>性质</th><th>最小项（Σm）</th><th>最大项（ΠM）</th></tr>
      <tr><td>取值</td><td>仅一组使之为<strong>1</strong></td><td>仅一组使之为<strong>0</strong></td></tr>
      <tr><td>两两关系</td><td>任意两个之<strong>积</strong>为0</td><td>任意两个之<strong>和</strong>为1</td></tr>
      <tr><td>全体</td><td>全体之<strong>和</strong>为1</td><td>全体之<strong>积</strong>为0</td></tr>
      <tr><td>编号关系</td><td>m<sub>i</sub> = <span style="text-decoration:overline">M</span><sub>i</sub>（i≠j时m<sub>i</sub>·m<sub>j</sub>=0）</td><td>M<sub>i</sub> = <span style="text-decoration:overline">m</span><sub>i</sub>（i≠j时M<sub>i</sub>+M<sub>j</sub>=1）</td></tr>
    </table>
  </div>
